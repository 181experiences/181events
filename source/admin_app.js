...
(function () {
    ...
    $("#evlist").innerHTML = gs.map(g => {
      ...
      <div class="edates" data-dates-for="${esc(g.key)}" style="display:none">${(g.upcoming.length ? g.upcoming : g.rows).map(r => `
        <div class="edrow"><span class="edwhen">${esc(fmt(r.Date))} &middot; ${esc(r.Start)}</span>
          ...
          <button class="mini" data-editrow="${esc(g.key)}|${esc(r.id)}">Edit this date</button>
          ${(r.Status || "Draft") === "Draft" ? `<button class="mini ghost" data-rowdelete="${esc(g.key)}|${esc(r.id)}" title="Drafts only: removes just this date; the rest of the series stays">Delete this date</button>` : ""}</div>`).join("")}${g.upcoming.length && g.rows.length > g.upcoming.length ? `
  ...
  function refreshEdActions() {
    ...
    // Delete lives here only for a Draft date opened on its own; a whole-series
    // edit points at the list's Delete draft, which takes the group at once.
    const series = !!(editing && editing.group && editing.group.series);
    const wholeSeries = series && $("#ed-occ").style.display !== "none" && $("#f-occ").value === "__all";
    $("#ed-delete").style.display = row && (row.Status || "Draft") === "Draft" && !wholeSeries ? "" : "none";
    $("#ed-delete").textContent = series ? "Delete this date" : "Delete draft";
  ...
  function renderDash() {
    ...
    const heads = sums.reduce((s, x) => s + x.resHeads, 0), waiting = sums.reduce((s, x) => s + x.resWaitHeads, 0);
    const staffAhead = sums.reduce((s, x) => s + x.roleHeads + x.roleWaitHeads, 0);
    $("#kpis").innerHTML = [
      ...
      ["RSVPs ahead", heads, waiting ? `${waiting} on waitlists${staffAhead ? " · staff not counted" : ""}`
        : staffAhead ? `resident heads · ${staffAhead} staff not counted` : "confirmed heads for upcoming dates"],
  ...
  function rsvpSummaries() {
    ...
    for (const r of rsvps) {
      ...
      if (!by.has(r.event_key)) by.set(r.event_key, {
        ...
        parties: 0, heads: 0, waitParties: 0, waitHeads: 0, arrivedHeads: 0, arrivedMarks: 0,
        resParties: 0, resHeads: 0, resWaitHeads: 0, resArrivedHeads: 0, roleHeads: 0, roleWaitHeads: 0, rows: [],
      });
      const s = by.get(r.event_key);
      s.rows.push(r);
      // A role account (no unit: the desk, leadership) holds real seats, so the
      // plain tallies keep everyone for capacity work; the res* tallies carry
      // residents only, and every reported figure reads from those.
      const role = !r.unit;
      if (r.status === "Waitlist") {
        s.waitParties++; s.waitHeads += r.count;
        if (role) s.roleWaitHeads += r.count; else s.resWaitHeads += r.count;
      } else {
        s.parties++; s.heads += r.count;
        if (role) s.roleHeads += r.count; else { s.resParties++; s.resHeads += r.count; }
        if (r.arrived != null) {
          s.arrivedHeads += r.arrived; s.arrivedMarks++;
          if (!role) s.resArrivedHeads += r.arrived;
        }
      }
    }
    const all = [...by.values()];
  ...
  function dialog(title, text, buttons, fields) {
    return new Promise(resolve => {
      $("#dlg-t").textContent = title;
      $("#dlg-p").innerHTML = text;
      $("#dlg-f").innerHTML = (fields || []).map(f =>
        `<div><label for="dlgf-${f.id}">${esc(f.label)}</label>
         <input id="dlgf-${f.id}" type="${f.type || "text"}" value="${esc(f.value || "")}" placeholder="${esc(f.placeholder || "")}"${f.type === "number" ? ' inputmode="numeric" min="0" max="6"' : ""}></div>`).join("");
      ...
      const done = v => {
        const f = {};
        for (const fl of fields || []) f[fl.id] = $("#dlgf-" + fl.id).value;
        wrap.style.display = "none"; wrap.onclick = null;
        resolve(fields ? { value: v, fields: f } : v);
      };
      wrap.onclick = ev => {
  ...
  const arriveBtn = r => r.status === "Waitlist" ? "" :
    `<button class="mini${r.arrived == null ? " ghost" : ""}" data-rarrive="${r.id}"
      title="${r.arrived == null ? "Check the party in at the door; a partial count is fine" : "Tap to adjust the count or the door note"}">${
      r.arrived == null ? "Check in" : `&#10003; ${r.arrived} of ${r.count} in`}</button>`;

  // The door dialog: whole party in one tap, a partial count when only some
  // arrived, and a margin note for the row ("third is parking the car").
  async function checkInFlow(row) {
    const res = await dialog(`Check in ${row.name}`,
      `${esc(row.event_title)} &middot; ${row.count === 1 ? "party of 1" : `party of ${row.count}`}. A partial count is fine; come back and adjust as the rest arrive.`,
      [
        { label: row.count === 1 ? `${row.name.split(" ")[0]} is here` : `All ${row.count} are here`, kind: "primary", value: "all" },
        { label: "Save the number below", value: "save" },
        ...(row.arrived != null ? [{ label: "Clear the check-in", value: "clear" }] : []),
        { label: "Not now", kind: "quiet", value: null },
      ],
      [
        { id: "n", label: "How many are in", value: String(row.arrived == null ? row.count : row.arrived), type: "number" },
        { id: "note", label: "Door note, optional", value: row.door_note || "", placeholder: "Two of three here; the third is parking" },
      ]);
    if (!res || !res.value) return;
    const body = { door_note: String(res.fields.note || "").trim() };
    if (res.value === "all") body.arrived = row.count;
    else if (res.value === "clear") body.arrived = null;
    else {
      const n = Math.round(Number(res.fields.n));
      if (!Number.isFinite(n) || n < 0 || n > 6) { toast("A number from 0 to 6.", "warn"); return; }
      body.arrived = n === 0 ? null : n;
    }
    api("/api/rsvps/" + row.id, { method: "PATCH", body: JSON.stringify(body) })
  ...
  function renderRsvps() {
    ...
    html += items.map(it => it.html !== null ? it.html : (s => {
      ...
      return `<div class="srow" style="cursor:pointer" data-rsvpkey="${esc(s.key)}"><span class="rchev${openRsvpDetails.has(s.key) ? " open" : ""}">&rsaquo;</span><span class="slab">${esc(fmt(s.date))}</span>
        ...
        <div style="display:flex;justify-content:flex-end;margin-bottom:6px"><button class="mini ghost" data-rsvpprint="${esc(s.key)}" title="The check-in sheet: names, parties, tick boxes">Print check-in list</button></div>
        ${s.rows.map(r => `<div class="srow"><span class="slab">${esc(r.unit || "Role")} &middot; ${esc(r.name)}</span>
          <span class="sgrow" style="font-size:14px;color:var(--ink-soft)">${r.status === "Waitlist" ? "Waitlist" : (s.type === "guest" ? `${r.count} guest${r.count === 1 ? "" : "s"}` : `party of ${r.count}`)}${r.names ? ` &middot; ${esc(r.names)}` : ""}${r.door_note ? ` &middot; <em>${esc(r.door_note)}</em>` : ""}</span>
    ...
    if (past.length) {
      ...
      <div id="pastrsvps" style="${pastRsvpsOpen ? "" : "display:none"}">
        ${past.map(s => {
          const staffTail = s.roleHeads ? ` &middot; +${s.roleHeads} staff` : "";
          const went = s.arrivedMarks ? `${s.resArrivedHeads} of ${s.resHeads} arrived${staffTail}`
            : s.type === "guest" ? `${s.heads} outside guests` : `${s.resHeads} confirmed${staffTail}`;
          ...
          return `<div class="srow" style="cursor:pointer" data-rsvpkey="${esc(s.key)}"><span class="rchev${openRsvpDetails.has(s.key) ? " open" : ""}">&rsaquo;</span><span class="slab">${esc(fmt(s.date))}</span>
            ...
            ${s.rows.map(r => `<div class="srow"><span class="slab">${esc(r.unit || "Role")} &middot; ${esc(r.name)}</span>
              <span class="sgrow" style="font-size:14px;color:var(--ink-soft)">${r.status === "Waitlist" ? "Waitlisted" : (s.type === "guest" ? `${r.count} guest${r.count === 1 ? "" : "s"}` : `party of ${r.count}`)}${r.names ? ` &middot; ${esc(r.names)}` : ""}${r.door_note ? ` &middot; <em>${esc(r.door_note)}</em>` : ""}</span>
  ...
  // The check-in sheet for a calendar event's RSVPs: the residents' side of
  // the door, the same paper the private guest list prints, boxes and all.
  function printRsvpList(key) {
    const all = rsvpSummaries();
    const sm = all.upcoming.find(x => x.key === key) || all.past.find(x => x.key === key);
    if (!sm) return;
    const conf = sm.rows.filter(r => r.status !== "Waitlist");
    const wait = sm.rows.filter(r => r.status === "Waitlist");
    const row = (r, i, pre) => `<tr><td><span class="box"></span></td><td class="n">${pre || ""}${i + 1}</td>
      <td>${esc(r.name)}${r.unit ? ` &middot; ${esc(r.unit)}` : ""}</td>
      <td>${sm.type === "guest" ? `${r.count} outside guest${r.count === 1 ? "" : "s"}` : `party of ${r.count}`}</td>
      <td class="note">${esc(r.names || "")}</td></tr>`;
    const heads = conf.reduce((a, r) => a + r.count, 0);
    const w = window.open("", "_blank");
    w.document.write(`<!DOCTYPE html><html><head><title>${esc(sm.title)}</title><style>
      ...
      .n{color:#7a7266;width:34px} .note{color:#7a7266;font-style:italic}
      @media print{ body{margin:16px} }
    </style></head><body>
    <h1>${esc(sm.title)} &middot; RSVP Check-In</h1>
    <p class="sub">${esc(fmt(sm.date))}</p>
    <p class="meta">${conf.length} ${conf.length === 1 ? "party" : "parties"}, ${heads} expected &middot; printed ${new Date().toLocaleString([], { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</p>
    <table><tr><th></th><th>#</th><th>Name</th><th>Party</th><th>With</th></tr>
    ${conf.map((r, i) => row(r, i)).join("")}
    </table>
    ${wait.length ? `<h1 style="font-size:16px;margin-top:26px">Waitlist &middot; seated only if room opens</h1>
    <table><tr><th></th><th>#</th><th>Name</th><th>Party</th><th>With</th></tr>
    ${wait.map((r, i) => row(r, i, "W")).join("")}
    </table>` : ""}
    </body></html>`);
    w.document.close();
    w.focus();
  ...
  function renderResidents() {
    ...
    // Four buttons, no more: the ones the desk reaches for at 10 pm (code by
    // email or on paper, a rotate), and Edit, which opens everything else
    // (end dates, standing, disable, delete) in the card above, one deliberate
    // step away from the row. Every row carries the same four controls, so the
    // columns line up; an unusable one just shades itself.
    ...
    const printBtn = p => p.status === "Active" && !p.expired
      ? `<button class="mini ghost" data-printone="${p.id}" title="Prints just this card, for the lobby printer">Print</button>`
      : `<span class="mini off" title="Available once the code is active again">Print</span>`;
  ...
  // With no argument, every active card at once; with one person, just their
  // card, for the lobby printer when the original walked off the desk.
  ...
  function renderEditorKit() {
    ...
    box.innerHTML = KITINFO.map(k => {
      ...
      const acts = [
        ...
        canva ? `<a class="mini ghost" href="${esc(canva)}" target="_blank" rel="noopener" title="Opens the design in Canva for editing">Canva</a>` : "",
        `<button class="mini ghost" data-acanva="${esc(mk)}|${k.slug}" title="Save the Canva address where this piece is edited; it saves to the ${isSeries ? "series kit, every date" : "kit"}">${canva ? "Edit link" : "Canva link"}</button>`,
      ].join("");
  ...
  function guestSection(b) {
    ...
    const total = confirmed.reduce((a, r) => a + heads(r), 0);
    const waitHeads = (g || []).filter(r => r.status === "Waitlist").reduce((a, r) => a + heads(r), 0);
  ...
  function printGuestList(b) {
    ...
    const lines = mk(g.filter(r => r.status !== "Waitlist"));
    const waitLines = mk(g.filter(r => r.status === "Waitlist"));
    const w = window.open("", "_blank");
    ...
    <p class="meta">${lines.length} expected guest${lines.length === 1 ? "" : "s"}${waitLines.length ? ` &middot; ${waitLines.length} on the waitlist below` : ""} &middot; guests give the event name at the door &middot; printed ${new Date().toLocaleString([], { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</p>
    ...
    ${waitLines.length ? `<h1 style="font-size:16px;margin-top:26px">Waitlist &middot; admitted only if seats open</h1>
    <table><tr><th></th><th>#</th><th>Guest</th><th></th></tr>
    ${waitLines.map(l => `<tr><td><span class="box"></span></td><td class="n">W${l.n}</td><td>${esc(l.name)}</td><td class="note">${esc(l.note)}</td></tr>`).join("")}
    </table>` : ""}
    </body></html>`);
    w.document.close();
    w.focus();
  ...
  // One date of a draft series can leave without the rest: the calendar-app
  // gesture ("delete only this event") for drafts. Server-side the DELETE is
  // Draft-only, so a published date can never fall through this door.
  async function deleteDraftRow(key, id) {
    const g = groups().find(x => x.key === key); if (!g) return;
    const r = g.rows.find(x => String(x.id) === String(id)); if (!r) return;
    const others = g.rows.length - 1;
    const tail = others === 0 ? "" : others === 1 ? " The other date stays." : ` The other ${others} dates stay.`;
    if (!confirm(`Delete ${fmt(r.Date)} from "${r.Title}"? Residents never saw it.${tail}`)) return;
    try { await api("/api/events/" + encodeURIComponent(r.id), { method: "DELETE" }); }
    catch (e) { toast(e.message, "warn"); return; }
    events = events.filter(e => e.id !== r.id);
    if (editing && editing.row && editing.row.id === r.id) { editing = null; go("events"); }
    renderAll();
    toast(others ? `${fmt(r.Date)} deleted; the series keeps its other date${others === 1 ? "" : "s"}.` : "Draft deleted.");
  }
  ...
  function moveAdminCal(step) {
    ...
    if (step) {
      ...
      // Finished months leave the calendar everywhere; the Archive keeps history.
      if (acalYM < t) acalYM = t;
    }
    renderAdminCal();
  }
  function renderAdminCal() {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      // The desk looks without editing: the same grid, and a chip opens the
      // view-only record below it rather than the editor.
      const chip = e => role === "desk"
        ? `<button class="acev st-${cls(e.Status)}${iso < t ? " past" : ""}" data-acalqv="${esc(String(e.id))}" title="A view-only look at ${esc(e.Title)}"><span class="at">${esc(e.Start || "")}</span> ${esc(e.Title)}</button>`
  ...
  function exportCsv() {
    ...
    const sums = rsvpSummary();
    rows.push([]);
    rows.push(["RSVPs by event, upcoming", "resident figures; staff role accounts sit in their own column"]);
    rows.push(["Date", "Event", "Type", "Parties", "Heads", "Capacity", "Waitlisted heads", "Staff heads"]);
    for (const s2 of sums) {
      const e = events.find(x => stem(x) === s2.key);
      rows.push([s2.date, s2.title, s2.type, s2.resParties, s2.resHeads, e && e.Capacity ? e.Capacity : "", s2.resWaitHeads, s2.roleHeads || ""]);
    }
    ...
    if (past.length) {
      rows.push([]);
      rows.push(["Attendance, past events", "resident figures; staff role accounts sit in their own column"]);
      rows.push(["Date", "Event", "Confirmed heads", "Arrived heads", "Parties marked", "Staff heads"]);
      for (const s2 of past) rows.push([s2.date, s2.title, s2.resHeads,
        s2.arrivedMarks ? s2.resArrivedHeads : "not marked", s2.arrivedMarks || "", s2.roleHeads || ""]);
    }
    rows.push([]);
    rows.push(["Messages"]);
  ...
  // ---------------------------------------------------------------- neighbor notes
  // The oversight card: every active note with its raised hands, a Remove for
  // EVERY tier (the desk holds the third shift, and the archive holds everyone
  // to account), the board's standing open switch (staff and owner only), and
  // Past notes: the full record of what faded or came down, by whom, when,
  // with Restore for anything taken down before its natural fade.
  let nbOpen = true;
  let nbPastOpen = false;
  async function loadNotes() {
    ...
    const whenOf = t => new Date(t).toLocaleString([], { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
    const byName = v => v === "resident" ? "its author" : esc(String(v || "staff").split("@")[0]);
    let html = "";
    if (!nbOpen) {
      html = '<div class="nodata">The board is resting. Residents see a quiet note saying so; tick the switch to reopen.</div>';
    } else if (!d.notes.length) {
      html = '<div class="nodata">Nothing pinned right now. Notes fade on their own after three days.</div>';
    } else {
      html = d.notes.map(n => `<div class="srow"><span class="slab" style="white-space:nowrap">${esc(n.who)}</span>
        ...
        <span class="sval" style="font-size:12px;color:var(--stone);white-space:nowrap">${esc(whenOf(n.created))}</span>
        <span class="eact"><button class="mini ghost" data-notedel="${n.id}" title="Comes down at once and keeps its record under Past notes, marked with your name">Remove</button></span></div>`).join("");
    }
    const past = d.past || [];
    if (past.length) {
      html += `<div class="srow" style="cursor:pointer" data-nbpast>
        <span class="sgrow" style="color:var(--stone)">Past notes &middot; ${past.length}</span>
        <span class="sval" style="color:var(--stone)">${nbPastOpen ? "Hide" : "Show"}</span></div>`;
      if (nbPastOpen) html += past.map(n => {
        const fadeMs = Date.parse(n.created) + 3 * 86400000;
        const state = n.deleted_at ? '<span class="pill unpublished">Taken down</span>' : '<span class="pill archived">Faded</span>';
        const bits = [`posted ${whenOf(n.created)}`];
        if (n.deleted_at) bits.push(`taken down ${whenOf(n.deleted_at)} by ${byName(n.deleted_by)}`);
        else bits.push(`faded ${whenOf(fadeMs)}`);
        if (n.restored_at) bits.push(`restored ${whenOf(n.restored_at)} by ${byName(n.restored_by)}`);
        if (n.hands.length) bits.push(`joining: ${esc(n.hands.join(", "))}`);
        const restorable = n.deleted_at && fadeMs > Date.now();
        return `<div class="srow" style="opacity:.85"><span class="slab" style="white-space:nowrap">${esc(n.who)}</span>
          <span class="sgrow" style="color:var(--ink-soft)">${esc(n.body)}<br><span style="font-size:12px;color:var(--stone)">${bits.join(" &middot; ")}</span></span>
          <span class="eact">${state}${restorable ? `<button class="mini ghost" data-noterestore="${n.id}" title="Back on the board with its original fade date">Restore</button>` : ""}</span></div>`;
      }).join("");
    }
    box.innerHTML = html;
  }
  ...
  async function deleteNote(id) {
    const ok = await dialog("Take this note down?",
      "It leaves the board at once and nothing is said to anyone. Its record stays under Past notes, marked with your name, and it can be restored any time before its natural fade date.",
      [{ label: "Take it down", kind: "primary", value: true }, { label: "Leave it", kind: "quiet", value: null }]);
    if (!ok) return;
    try { await api("/api/notes/" + encodeURIComponent(id), { method: "DELETE" }); loadNotes(); toast("Down, with its record kept under Past notes."); }
    catch (e) { toast(e.message, "warn"); }
  }

  async function restoreNote(id) {
    try { await api("/api/notes/" + encodeURIComponent(id), { method: "PATCH", body: JSON.stringify({ restore: true }) }); loadNotes(); toast("Back on the board, fading on its original date."); }
    catch (e) { toast(e.message, "warn"); }
  }
  ...
  document.addEventListener("click", ev => {
    ...
    if (r) {
      ...
      else if (r.dataset.rsvpprint) printRsvpList(r.dataset.rsvpprint);
      else if (r.dataset.notedel) deleteNote(r.dataset.notedel);
      else if (r.dataset.noterestore) restoreNote(r.dataset.noterestore);
      else if (r.dataset.nbpast !== undefined) { nbPastOpen = !nbPastOpen; loadNotes(); }
      else if (r.dataset.rarrive) {
        const row = rsvps.find(x => String(x.id) === r.dataset.rarrive);
        if (row) checkInFlow(row);
      }
      else if (r.dataset.rsvpkey) {
        // One event open at a time: opening the next closes the last, so the
        // list stays a list even on a night with fifty names.
    ...
    if (b.dataset.eddelete !== undefined) {
      if (editing && editing.group && editing.row) deleteDraftRow(editing.group.key, editing.row.id);
      return;
    }
    ...
    if (b.dataset.acalqv) {
      const row = events.find(e => String(e.id) === b.dataset.acalqv);
      const qv = $("#acal-qv");
      if (row && qv) {
        const g = { head: row, rows: [row], upcoming: row.Date >= today() ? [row] : [], series: false,
                    status: row.Status || "Draft", mixed: false };
        qv.innerHTML = `<div class="flrow" style="margin:16px 0 6px"><span style="font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--stone);font-weight:600">${esc(row.Title)} &middot; ${esc(fmt(row.Date))}</span></div><div class="qv">${qvHtml(g)}</div>`;
        qv.style.display = "";
